...
for the files in take5, the code is read starting from the main file, with the dealer being a main component and the components file containing the base object classes. We adjusted our interface to put more computational stress on the main program and less on the dealer. We realized our interface was missing class names and so implemented the program with class names we deemed appropriate.
