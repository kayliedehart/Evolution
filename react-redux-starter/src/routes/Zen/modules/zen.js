...
// TODO: check that previous request is finished (pass in getState to redux-thunk)
...
const ZEN_ACTION_HANDLERS = {
  ...
  [SAVE_CURRENT_ZEN]: (state: ZenStateObject): ZenStateObject => {
    return ((state.current != null) && !state.saved.includes(state.current)) ? ({ ...state, saved: state.saved.concat(state.current) }) : state
  }
}
